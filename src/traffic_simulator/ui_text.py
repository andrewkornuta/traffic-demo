...
METRIC_COPY = {
    ...
    "throughput": {"label": "Cars Through", "unit": "", "better": "higher"},
...
def summarize_mutation(mutation: Mutation) -> str:
    ...
    if mutation.mutation_type == "replace_signal_with_roundabout":
        return f"Replace the traffic light at {params['node_id']} with a roundabout."
    if mutation.mutation_type == "add_connector":
        return f"Add a new road connection from {params['source']} to {params['target']}."
    if mutation.mutation_type == "remove_connector":
        return f"Remove the road connection on {params['edge_id']}."
    if mutation.mutation_type == "close_edge":
        return f"Close road segment {params['edge_id']} to simulate a disruption."
    if mutation.mutation_type == "change_speed_limit":
        return f"Change the speed limit on {params['edge_id']}."
    if mutation.mutation_type == "change_lane_count":
        return f"Change {params['edge_id']} to {params['lane_count']} lane(s)."
    if mutation.mutation_type == "change_signal_plan":
        return f"Change the light timing plan at {params['node_id']}."
